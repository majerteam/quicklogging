...
def test_output(streamconfig, module_name, message):
    """generic testing: give a stream, a module that logs and a message
    """
...
_TEST_CATCHPRINT_MODULE_TEMPLATE = (
    ...
    "    return my_logger.name, [(h.stream, h.stream.buckets) "
...
def test_catch_prints(streamconfig, module_name):
    ...
    message = "bonjour depuis {}".format(module_name)
    ...
    expected_re = r"INFO:{}:\d+:{}".format(module_name, message)
...
def test_hierarchical_catch_print(streamconfig):
    ...
    assert re.match(
        r"INFO:parent.child:\d+:message seen by parent",
